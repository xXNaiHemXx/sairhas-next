...
interface StudentLoginProps {
  onLogin: (pairKey: string, role: 'Y1' | 'Y2', studentId: string) => void;
}
...
export default function StudentLogin({ onLogin }: StudentLoginProps) {
  ...
  const handleLoginClick = () => {
    ...
    console.log('🔍 digits length:', digits.length); // ✅ Debug log
  ...
  const handleConfirmLogin = async () => {
    ...
    try {
      ...
      if (result.pair) {
        const role = result.pair.y2_id === studentId ? 'Y2' : 'Y1';
        onLogin(result.pair.pair_key, role, studentId);
      } else if (result.parsed) {
        const role = result.parsed.role;
        if (role === 'Y2') {
          onLogin('', 'Y2', studentId);
        } else {
          onLogin('', 'Y1', studentId);
        }
      } else {
  ...
  // ✅ ตรวจสอบว่าสามารถกด Login ได้หรือไม่
  const isLoginDisabled = isLoading || studentId.replace(/\D/g, '').length < 11;

  // Popup Component
  const ConfirmPopup = () => {
    if (!showConfirmPopup || !parsedData) return null;

    const roleLabel = parsedData.role === 'Y2' ? 'พี่ (Y2)' : 'น้อง (Y1)';
    const roleBadge = parsedData.role === 'Y2' ? 'badge-y2' : 'badge-y1';
            ...
            <div className="popup-info-grid">
              ...
              <div className="popup-info-item">
                ...
                <span className="popup-info-value">
                  <span className={`badge ${roleBadge}`}>{parsedData.role}</span>
                  {' '}{roleLabel}
                </span>
              </div>
              <div className="popup-info-item">
          ...
          <div className="input-group">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setStudentId(value);
                setError('');
                console.log('📝 Current studentId:', value); // ✅ Debug log
              }}
